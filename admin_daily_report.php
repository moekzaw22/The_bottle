...
include('admin_navbar.php');
 ...
 .container-1{
	...
	padding-right:5	0px;
 	...
 	<form action="admin_daily_report.php" method="POST">
 ...
 <table class="table">
 	...
 	$select="SELECT Date,SUM(totalprice) AS totalp, SUM(Buy_Quantity) AS qtytotal FROM purchase GROUP BY Date ORDER BY Date = '$sltdate'";
 		...
 		$select="SELECT Date,SUM(totalprice) AS totalp, SUM(Buy_Quantity) AS qtytotal, SUM(Profit) AS ttlpro FROM purchase WHERE status = 'Confirmed' AND Date = '$txtdate' GROUP BY Date ORDER BY Date DESC";
			$select_query=mysqli_query($connect,$select);
			$count=mysqli_num_rows($select_query);
			for ($i=0; $i < $count ; $i++) { 
				$select_array=mysqli_fetch_array($select_query);
 	...
 	$select="SELECT Date,SUM(totalprice) AS totalp, SUM(Buy_Quantity) AS qtytotal, SUM(Profit) AS ttlpro FROM purchase WHERE status = 'Confirmed' GROUP BY Date ORDER BY Date DESC";
			$select_query=mysqli_query($connect,$select);
			$count=mysqli_num_rows($select_query);
			for ($i=0; $i < $count ; $i++) { 
				$select_array=mysqli_fetch_array($select_query);
